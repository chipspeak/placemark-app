...
<!-- Displaying weather details including using the getIconUrl from utils to display the icon returned by openweather -->
{#if weatherDataMap[placemark._id]?.weather}
  {#each weatherDataMap[placemark._id].weather as weather}
    <!-- svelte-ignore a11y-invalid-attribute -->
    <a title="Expand/Collapse Weather" aria-label="more options" data-action="collapse" href="#" on:click={(event) => toggleWeatherCollapse(event, `weather-${placemark._id}`)}>
